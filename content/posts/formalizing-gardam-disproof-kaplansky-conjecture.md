...
A convenient way to browse the formalization is via the [documentation](http://math.iisc.ac.in/~gadgil/unit_conjecture/GardamTheorem.html) with links to the source code, generated by `docgen4`. The package `Polylean` contains all the modules; alternatively, the docs can be navigated by following imports.
