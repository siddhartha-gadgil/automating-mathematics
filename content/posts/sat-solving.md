...
Suppose the given `SAT` problem has no solution, as discovered by our recursive algorithm. We can enhance our algorithm to in this case give a proof using resolution of a contradiction starting with the given clauses `$E$`. We sketch this here. The main step is the _lifting_ of a proof from a restricted problem. We shall denote the set of clauses deduced by resolution from a set `$E$` of clauses by `$D(E)$`.
...
Now consider the general case with `$n$` variables, and assume that our algorithm gives either a solution or deduces a contradiction using resolution whenever we have fewer than `$n$` variables. Pick a variable `$P$` and assign a truth value to this, say true. As above, we get a restricted problem `$SAT(V\setminus\{P\}, \rho(E, P))$` not involving the variable `$P$`.

Suppose the original problem has no solution, then the restricted problem `$SAT(V\setminus\{P\}, \rho(E, P))$` does not either. By the induction hypothesis, using resolution we can deduce a contradiction starting with  `$\rho(E, P)$`, i.e., `$\bot\in  D(\rho(E, P))$` (where `$\bot$` is the empty clause).
We cannot conclude that `$\bot\in D(E)$`. Nevertheless we can lift the proof of a contradiction to something useful.
...
This claim can be proved by induction on the number of steps in the deduction using resolution. For `$\bot$` steps, we just get elements of `$\rho(E, P)$`, for which we have already observed the claim. On the other hand, if `$C \in D(\rho(E, P))$` is deduced in `$n$` steps, then `$C$` can be deduced from clauses `$C_1$` and `$C_2$` by resolution, and the clauses `$C_i$` can be deduced from `$\rho(E, p)$` in fewer than `$n$` steps.
...
Furthermore, we would like the compiler to be able to check that the program terminates for any valid input, and has correct output of one of the above forms. There is no obstruction in principle to doing this, but some work is needed.
